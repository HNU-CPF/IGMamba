...
def save_imgs(img, msk, msk_pred, i, save_path, datasets, threshold=0.5, test_data_name=None):
    img = img.squeeze(0).permute(1,2,0).detach().cpu().numpy() #去掉轴的厚度，并将HWC换为WCH,
    ...
    if datasets == 'retinal':
        ...
        msk_pred = np.where(np.squeeze(msk_pred, axis=0) > 0.2, 1, 0) #素值大于该阈值，则被标记为 1，否则被标记为 0
...
class CeDiceLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        return loss
class valDiceLoss(nn.Module):

    def __init__(self, wb=1):
        super(valDiceLoss, self).__init__()
        self.wb = wb
        self.dice = DiceLoss()
    def forward(self, pred, targer):
        dice_loss = self.dice(pred, targer)
        loss = self.wb * dice_loss
        return loss
    ...
    def __init__(self, data_name, train=True):
        if data_name == 'isic18':
            ...
            if train:
                ...
                self.mean = 149.8485
                self.std = 35.3346
        elif data_name == 'Dataset_NulnsSeg':
            if train:
                self.mean = 159.922
                self.std = 28.871
            else:
