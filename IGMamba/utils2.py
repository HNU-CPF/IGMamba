...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        if len(label.shape) > 3 or len(label.shape) < 2:
            raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 3 dims or 2 dims.\n"))
...
class RandomRotationScale(object):
    ...
    def __call__(self, img, target):
        ...
        input_size =  self.size     #因为dence-net下采样了五次
        #  randomly scale
        ...
        if len(cropp_tumor.shape) == 2:
            cropp_tumor = np.expand_dims(cropp_tumor, axis=2)
...
class RandomRotationScaleSmall(object):
    ...
    def __call__(self, img, target):
        ...
        # random crop
        random_x = np.random.randint(0, img.shape[0] - deps + 1)
        random_y = np.random.randint(0, img.shape[1] - rows + 1)
        cropp_img = img[random_x: random_x + deps, random_y: random_y + rows, :].copy()
        cropp_tumor = target[random_x: random_x + deps, random_y: random_y + rows].copy()
