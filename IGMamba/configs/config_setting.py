...
class setting_config:
    ...
    if datasets == 'isic18':
        ...
        data_path = './data/Glas/'
    elif datasets == 'Dataset_NulnsSeg':
        data_path = './data/Dataset_NulnsSeg/'
    elif datasets == 'lijieyu':
        data_path = './data/lijieyu/'
    else:
        raise Exception('datasets in not right!')

    criterion = BceDiceLoss(wb=1, wd=1)

    # criterion_val = valDiceLoss(wb=1)
    ...
    work_dir = 'results/' + 'EA+GA'+ network + '_' + datasets + '_' + datetime.now().strftime('%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'
    ...
    if datasets == 'hou':
        ...
        train_transformer = transform.Compose([
            ...
            transform.RandomRotationScale(480),
            ...
            # myResize(input_size_h, input_size_h)
        ])
