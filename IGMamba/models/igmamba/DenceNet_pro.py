...
from .layers import SaveFeatures,UnetBlock_,UnetBlock,UnetBlock3d_,UnetBlock3d
import torch
...
class SPPblock(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.layer1.permute(0, 2, 3, 1).contiguous()
        # print(self.pool1(x).shape,self.conv(self.pool1(x)).shape,self.layer1.shape)
        self.layer2 = F.upsample(self.conv(self.pool2(x)), size=(h, w), mode='bilinear')
        # self.layer2.permute(0, 2, 3, 1).contiguous()
        self.layer3 = F.upsample(self.conv(self.pool3(x)), size=(h, w), mode='bilinear')
        # self.layer3.permute(0, 2, 3, 1).contiguous()
        self.layer4 = F.upsample(self.conv(self.pool4(x)), size=(h, w), mode='bilinear')
        # self.layer4.permute(0, 2, 3, 1).contiguous()
...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes,ratio=16):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)   #提取全局信息
        self.max_pool = nn.AdaptiveMaxPool2d(1)

        self.fc1 = nn.Conv2d(in_planes, in_planes // ratio, 1, bias=False)
        self.relu1 = nn.ReLU()                                                           #增加网络的非线性拟合能力
